export const PROJECTS = [
  {
    id: "realtime-chat-app",
    slug: "realtime-chat-app",
    title: "RealTime Chat_App",
    category: "FULL STACK APPLICATION",
    date: "2026",
    shortDescription:
      "A full-stack real-time chat application built with the MERN stack and Socket.io for instant messaging.",
    description:
      "A full-stack real-time chat application built with the MERN stack (MongoDB, Express.js, React, Node.js) and Socket.io for instant messaging. Features user authentication, profile management, and live chat with online status updates.",
    image:
      "https://images.unsplash.com/photo-1611162617474-5b21e879e113?q=80&w=2000&auto=format&fit=crop",
    tech: [
      "React + Vite",
      "Framer Motion + GSAP",
      "ReactBits",
      "Node.js",
      "Express.js",
      "MongoDB",
      "Socket.io",
    ],
    liveUrl: "https://chatt-app-iota.vercel.app/",
    githubUrl: "https://github.com/Abinashrout244/ChattAPP",
    featured: true,
    problem:
      "Users need a fast and reliable platform where they can communicate in real time while seeing online status updates.",
    solution:
      "Built a MERN-based real-time chat platform using Socket.io for instant communication, with authentication and profile management.",
    features: [
      "Real-time messaging with Socket.io",
      "User authentication",
      "User profile management",
      "Online status updates",
    ],
    challenges:
      "Managing real-time communication and keeping chat and online-status data synchronized between users.",
    learnings:
      "Gained practical experience with Socket.io, real-time communication, authentication, and MERN stack architecture.",
    futureImprovements:
      "Planning to add media sharing, message reactions, typing indicators, and group conversations.",
    status: "COMPLETED",
    platform: "Web",
    role: "Full Stack Developer",
    duration: "3 Months",
    gallery: [
      "https://images.unsplash.com/photo-1611162617474-5b21e879e113?q=80&w=2000&auto=format&fit=crop",
    ],
  },

  {
    id: "note-app",
    slug: "note-app",
    title: "Note_App",
    category: "PRODUCTIVITY TOOL",
    date: "2025",
    shortDescription:
      "A full-stack note-taking application with authentication, protected routes, and complete note management.",
    description:
      "I developed a full-stack Note Taking application where users can securely create, edit, and delete notes. The app includes authentication, protected routes, and real-time data management using modern MERN stack technologies.",
    image:
      "https://images.unsplash.com/photo-1498050108023-c5249f4df085?q=80&w=2000&auto=format&fit=crop",
    tech: [
      "React + Vite",
      "Tailwind CSS",
      "Node.js",
      "Express.js",
      "MongoDB",
      "Redux Toolkit",
    ],
    liveUrl: "https://note-app-lilac-two.vercel.app/",
    githubUrl: "https://github.com/Abinashrout244/Note_app",
    featured: true,
    problem:
      "Users need a simple and secure way to create, edit, delete, and manage their personal notes.",
    solution:
      "Developed a full-stack note-taking application with authentication, protected routes, Redux Toolkit, and MongoDB for data management.",
    features: [
      "Create, edit, and delete notes",
      "Secure user authentication",
      "Protected routes",
      "Real-time data management",
    ],
    challenges:
      "Managing authentication, protected routes, and keeping note data synchronized between the frontend and backend.",
    learnings:
      "Improved my understanding of MERN stack development, Redux Toolkit, authentication, and REST API integration.",
    futureImprovements:
      "Planning to add note categories, search functionality, markdown support, and cloud synchronization.",
    status: "COMPLETED",
    platform: "Web",
    role: "Full Stack Developer",
    duration: "2 Months",
    gallery: [
      "https://images.unsplash.com/photo-1498050108023-c5249f4df085?q=80&w=2000&auto=format&fit=crop",
    ],
  },

  {
    id: "netflix-clone",
    slug: "netflix-clone",
    title: "Netflix_Clone",
    category: "WEB APPLICATION",
    date: "2025",
    shortDescription:
      "A Netflix-inspired streaming platform using React, Firebase, Tailwind CSS, and the TMDB API.",
    description:
      "I developed a Netflix-inspired streaming platform using modern web technologies, integrating data from The Movie Database (TMDb) API to dynamically fetch and display movies and TV shows.",
    image:
      "https://images.unsplash.com/photo-1586899028174-e7098604235b?q=80&w=2000&auto=format&fit=crop",
    tech: ["HTML", "Tailwind CSS", "React + Vite", "Firebase", "TMDB API"],
    liveUrl:
      "https://netflix-clone-jiun-kxtb9q9ot-abinash-routs-projects.vercel.app/",
    githubUrl: "https://github.com/Abinashrout244/Netflix_clone",
    featured: true,
    problem:
      "Building a movie streaming interface requires dynamic movie data, authentication, responsive layouts, and efficient API integration.",
    solution:
      "Created a Netflix-inspired interface using React and Tailwind CSS while integrating TMDB API for dynamic movie and TV show data and Firebase for authentication.",
    features: [
      "Dynamic movie and TV show data",
      "TMDB API integration",
      "Firebase authentication",
      "Responsive Netflix-inspired UI",
    ],
    challenges:
      "Working with external movie APIs and creating a responsive streaming-style interface with large amounts of dynamic content.",
    learnings:
      "Learned more about API integration, Firebase authentication, React component architecture, and responsive UI development.",
    futureImprovements:
      "Planning to add watchlists, personalized recommendations, advanced search, and improved video playback functionality.",
    status: "COMPLETED",
    platform: "Web",
    role: "Frontend Developer",
    duration: "2 Months",
    gallery: [
      "https://images.unsplash.com/photo-1586899028174-e7098604235b?q=80&w=2000&auto=format&fit=crop",
    ],
  },

  {
    id: "ec-shopcart",
    slug: "ec-shopcart",
    title: "Ec-ShopCart",
    category: "E-COMMERCE APPLICATION",
    date: "2025",
    shortDescription:
      "A simple and user-friendly shopping website for browsing products and managing a shopping cart.",
    description:
      "ShopCart is a simple and user-friendly shopping website that allows users to browse products, view details, and manage their cart with a clean and modern interface.",
    image:
      "https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?q=80&w=2000&auto=format&fit=crop",
    tech: ["HTML", "Tailwind CSS", "React + Vite", "Firebase"],
    liveUrl: "https://avi-ec-vmhw.vercel.app/",
    githubUrl: "https://github.com/Abinashrout244/AVI-Ec",
    featured: true,
    problem:
      "Users need a simple and clean shopping interface where they can browse products, view details, and manage their cart.",
    solution:
      "Built a responsive e-commerce frontend using React, Vite, Tailwind CSS, and Firebase.",
    features: [
      "Product browsing",
      "Product details",
      "Shopping cart management",
      "Firebase integration",
    ],
    challenges:
      "Creating a smooth shopping experience while managing product state and cart interactions.",
    learnings:
      "Strengthened my understanding of React, component-based UI development, Tailwind CSS, and Firebase.",
    futureImprovements:
      "Planning to add payment integration, user order history, product search, filters, and backend product management.",
    status: "COMPLETED",
    platform: "Web",
    role: "Frontend Developer",
    duration: "1.5 Months",
    gallery: [
      "https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?q=80&w=2000&auto=format&fit=crop",
    ],
  },

  {
    id: "flower-shop",
    slug: "flower-shop",
    title: "FlowerShop",
    category: "WEB APPLICATION",
    date: "2025",
    shortDescription:
      "A charming flower shop website featuring fresh blooms, elegant bouquets, and seasonal arrangements.",
    description:
      "A charming Flower Shop website offering a variety of fresh blooms, elegant bouquets, and seasonal arrangements with a simple, user-friendly browsing experience.",
    image:
      "https://images.unsplash.com/photo-1497250681960-ef046c08a56e?q=80&w=2000&auto=format&fit=crop",
    tech: [
      "Tailwind CSS",
      "React + Vite",
      "Swiper",
      "Framer Motion",
      "Lucide React",
    ],
    liveUrl: "https://flowershop-web-v0-owzr.vercel.app/",
    githubUrl: "https://github.com/Abinashrout244/Flowershop-web-v0",
    featured: true,
    problem:
      "A flower shop needs an attractive and easy-to-use interface that presents different flowers and arrangements clearly.",
    solution:
      "Created a responsive flower shop website with modern animations, sliders, and a clean visual design.",
    features: [
      "Responsive flower shop interface",
      "Interactive product sections",
      "Swiper image sliders",
      "Smooth Framer Motion animations",
    ],
    challenges:
      "Creating a visually appealing interface while maintaining smooth animations and responsive layouts.",
    learnings:
      "Improved my skills with Framer Motion, Swiper, Lucide React, responsive design, and modern React UI development.",
    futureImprovements:
      "Planning to add shopping cart functionality, authentication, checkout, and online payment integration.",
    status: "COMPLETED",
    platform: "Web",
    role: "Frontend Developer",
    duration: "1 Month",
    gallery: [
      "https://images.unsplash.com/photo-1497250681960-ef046c08a56e?q=80&w=2000&auto=format&fit=crop",
    ],
  },

  {
    id: "tictactoe",
    slug: "tictactoe",
    title: "TicTacToe",
    category: "GAME APPLICATION",
    date: "2025",
    shortDescription:
      "A simple Tic-Tac-Toe game with clean UI, smooth gameplay, and instant win detection.",
    description:
      "A simple Tic-Tac-Toe game built with clean UI and smooth gameplay, allowing two players to compete with instant win detection.",
    image:
      "https://images.unsplash.com/photo-1606092195730-5d7b9af1efc5?q=80&w=2000&auto=format&fit=crop",
    tech: ["HTML", "Tailwind CSS", "React + Vite"],
    liveUrl: "https://tic-tac-toe-app-v0.vercel.app/",
    githubUrl: "https://github.com/Abinashrout244/Tic-Tac-Toe-app-v0",
    featured: false,
    problem:
      "Create a simple browser-based game that provides an interactive two-player experience with immediate game results.",
    solution:
      "Built a lightweight Tic-Tac-Toe application using React with state management for turns, moves, and win detection.",
    features: [
      "Two-player gameplay",
      "Instant win detection",
      "Clean responsive UI",
      "Interactive game board",
    ],
    challenges:
      "Implementing the game logic correctly while keeping the UI simple and responsive.",
    learnings:
      "Improved my understanding of React state, event handling, conditional rendering, and game logic.",
    futureImprovements:
      "Planning to add AI gameplay, score tracking, sound effects, and different game modes.",
    status: "COMPLETED",
    platform: "Web",
    role: "Frontend Developer",
    duration: "1 Week",
    gallery: [
      "https://images.unsplash.com/photo-1606092195730-5d7b9af1efc5?q=80&w=2000&auto=format&fit=crop",
    ],
  },

  {
    id: "fake-api",
    slug: "fake-api",
    title: "Fake-API",
    category: "WEB APPLICATION",
    date: "2025",
    shortDescription:
      "A lightweight API-based application for browsing and displaying dynamic content.",
    description:
      "AJ Tube is a simple video-browsing platform that fetches and displays content using API data, offering a smooth and lightweight YouTube-style experience.",
    image:
      "https://images.unsplash.com/photo-1492724441997-5dc865305da7?q=80&w=2000&auto=format&fit=crop",
    tech: ["Tailwind CSS", "Framer Motion", "Lucide React", "Redux Toolkit"],
    liveUrl: "https://fa-ke-api-v0.vercel.app",
    githubUrl: "https://github.com/Abinashrout244/FAKe_API_V0",
    featured: false,
    problem:
      "Users need a simple interface for browsing dynamic API-based content without unnecessary complexity.",
    solution:
      "Built a lightweight React application using API data with Redux Toolkit for state management and modern UI libraries for interaction and animations.",
    features: [
      "API-based dynamic content",
      "Redux Toolkit state management",
      "Responsive interface",
      "Framer Motion animations",
    ],
    challenges:
      "Managing asynchronous API data and maintaining consistent application state while displaying dynamic content.",
    learnings:
      "Learned more about API integration, Redux Toolkit, Framer Motion, and reusable React components.",
    futureImprovements:
      "Planning to improve API functionality, add authentication, search, filtering, and personalized content.",
    status: "COMPLETED",
    platform: "Web",
    role: "Frontend Developer",
    duration: "1 Month",
    gallery: [
      "https://images.unsplash.com/photo-1492724441997-5dc865305da7?q=80&w=2000&auto=format&fit=crop",
    ],
  },

  {
    id: "aj-tube",
    slug: "aj-tube",
    title: "AJ Tube",
    category: "VIDEO PLATFORM",
    date: "2025",
    shortDescription:
      "A simple video-browsing platform offering a lightweight YouTube-style experience.",
    description:
      "AJ Tube is a simple video-browsing platform that fetches and displays content using API data, offering a smooth and lightweight YouTube-style experience.",
    image:
      "https://images.unsplash.com/photo-1492619375914-88005aa9e8fb?q=80&w=2000&auto=format&fit=crop",
    tech: ["HTML", "Tailwind CSS", "JavaScript"],
    liveUrl: "https://abinashrout244.github.io/AJ_Tube_clone/",
    githubUrl: "https://github.com/Abinashrout244/AJ_Tube_clone",
    featured: false,
    problem:
      "Build a lightweight video browsing experience using frontend technologies and API-based content.",
    solution:
      "Created a YouTube-inspired video browsing interface using HTML, Tailwind CSS, and JavaScript.",
    features: [
      "Video browsing interface",
      "API-based content",
      "Responsive layout",
      "YouTube-style UI",
    ],
    challenges:
      "Creating a responsive video browsing interface while handling dynamically loaded content with JavaScript.",
    learnings:
      "Strengthened my fundamentals in JavaScript, API handling, DOM manipulation, and responsive UI development.",
    futureImprovements:
      "Planning to add user authentication, video search, categories, playlists, and a backend system.",
    status: "COMPLETED",
    platform: "Web",
    role: "Frontend Developer",
    duration: "1 Month",
    gallery: [
      "https://images.unsplash.com/photo-1492619375914-88005aa9e8fb?q=80&w=2000&auto=format&fit=crop",
    ],
  },
];
